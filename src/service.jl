...
"""
get_metrics(c::QdrantConnection=get_client()) = parse_response(request(HTTP.get, c, "/metrics"))
...
"""
get_telemetry(c::QdrantConnection=get_client()) = execute(HTTP.get, c, "/telemetry")
